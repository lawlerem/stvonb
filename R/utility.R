...
#' Retrieve an age variable from a formula and a data.frame.
#'
#' @param x A formula object with an age(...) function containing one
#'   entry.
#' @param data A data.frame containing the age information.
#'
#' @return A data.frame with a single column for the sample size.
#'
#' @noRd
age_from_formula<- function(x, data, unique_vars = FALSE) {
  data<- data.frame(data)
  # Get out just the "age" term from the formula (as a character string)
  the_terms<- terms(x, specials = c("time", "space", "age"))
  the_idx<- attr(the_terms, "specials")$age
  if( length(the_idx) == 0 ) {
    stop("Must supply an age variable.")
    return(age)
  } else if( length(the_idx) > 1 ) {
    stop("Multiple `age` terms found in formula. Only one is allowed.")
  } else {}
  ...
  if( length(var_call) > 1 ) {
    stop("Only one age variable allowed.")
  } else {}
  ...
  if( length(missing_vars) > 0 ) {
    stop(paste(
      "Age variable not present in data: \n",
      paste(missing_vars, collapse=", ")
    ))
  } else {}


  age<- data[, char_vars, drop = FALSE]
...
#'   argument must be one those listed in get_stvonb_distributions().
...
covariance_from_formula<- function(x) {
  ...
  space<- function(covariance, nu) {
    ...
    match<- pmatch(
      covariance,
      get_stvonb_distributions("covariance"),
      duplicates.ok = TRUE
    )
    covar<- unname(get_stvonb_distributions("covariance")[match])
...
#' List of stvonb covariance functions
#'
...
#'
#' @export
get_stvonb_distributions<- function(
    which = "covariance"
  ) {
...
mean_design_from_formula<- function(x, data, return = "model.matrix") {
  ...
  the_terms<- delete.response(
    terms(x, specials = c("time", "space", "age"))
  )
...
time_from_formula<- function(x, data, return="vector") {
  ...
  time<- function(time_var, type = "ar1") {
    ...
    # Why is this not in get_stvonb_distributions?
